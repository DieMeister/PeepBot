...
class Peep(commands.Cog):
    ...
    async def psps(self, ctx: "Context"):
        ...
        last_member_count = dt.strptime(member["execute_psps_timestamp"], "%Y-%m-%dT%H:%M:%S").replace(tzinfo=datetime.UTC)
        last_guild_count = dt.strptime(guild["last_peep"], "%Y-%m-%dT%H:%M:%S").replace(tzinfo=datetime.UTC)
        ...
        member["execute_psps_timestamp"] = timestamp.strftime("%Y-%m-%dT%H:%M:%S")
        guild["last_peep"] = timestamp.strftime("%Y-%m-%dT%H:%M:%S")

        peep = "<:sinahehPeep:1316106573281492992>"
        scratch = "<:sinahehFlipoff:1266850082393821309>"
        no_peep = "<:sinahehL:1266850087250690079>"
        ...
        logic.logging("info", "peep", "Member used !psps", {
            "command": {
                    "guild": ctx.guild.id,
